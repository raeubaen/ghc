    ...
    def get_root_object(self, run_info):
        ...
        try:
            ...
            try:
                        ...
                        def clamp_values(obj):
                            if isinstance(obj, dict):
                                return {k: clamp_values(v) for k, v in obj.items()}
                            elif isinstance(obj, list):
                                return [clamp_values(v) for v in obj]
                            elif isinstance(obj, float):
                                if abs(obj) > 1e10:
                                    return 1e10
                                return obj
                            elif isinstance(obj, str) and obj.lower() in ("inf", "infinity", "nan"):
                                return 0
                            return obj

                        json_object = clamp_values(json_object)

                        json_object["fZmax"] = json_object.get("fZmax", 1e10)
                        json_object["fZmin"] = json_object.get("fZmin", 0)

                        return ROOT.TBufferJSON.ConvertFromJSON(json.dumps(json_object))

            except Exception as e:
                print(f"Fallback JSON parsing failed for run {run_info['run']}: {e}")
                return None

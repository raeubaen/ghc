...
def read_hist_EE(one_run_root_object, supermodule, EEchannels, ecal):
    nbinsx = one_run_root_object.GetNbinsX()
    nbinsy = one_run_root_object.GetNbinsY()
    for iy in range(1, nbinsy+1):
        for ix in range(1, nbinsx+1):
...
class Laser3AmplitudeOverPN(Plugin):
    ...
    def process_one_run(self, run_info):
        ...
        for i, EBsupermodule in enumerate(EBsupermodules_list):
            ...
            read_hist_EB(one_run_root_object, EBsupermodule, Ichannels, Lchannels, ecal)
        ...
        for i, EEsupermodule in enumerate(EEsupermodules_list):
            ...
            self.plot_name = f"EELT amplitude over PN {EEsupermodule} L3"
            EEchannels = {"label": [], "value": [], "LightMR": []}
            one_run_root_object = self.get_root_object(run_info)
            read_hist_EE(one_run_root_object, EEsupermodule, EEchannels, ecal)
        ...
        run_df.to_csv(f'/afs/cern.ch/user/c/charlesf/ghc/GoodHealthCheck/laser-proc/output/LT_AmpOverPN_{run_info["run"]}.csv', index = False) 

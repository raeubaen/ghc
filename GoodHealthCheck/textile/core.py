...
try:
    ...
    upper_re_s = "".join([chr(c) for c in
        range(maxunicode) if chr(c).isupper()])
...
class Textile(object):
    ...
    def placeNoteLists(self, text):
        ...
        if self.notes:
            o = OrderedDict()
            for label, info in list(self.notes.items()):
                if 'seq' in info:
                    i = info['seq']
                    info['seq'] = label
                    o[i] = info
            ...
            if o:
                # sort o by key
                o = OrderedDict(sorted(list(o.items()), key=lambda t: t[0]))
            self.notes = o
    ...
    def fNoteLists(self, match):
        ...
        if index not in self.notelist_cache:
            o = []
            if self.notes:
                for seq, info in list(self.notes.items()):
            ...
            if '+' == extras and self.unreferencedNotes:
                for seq, info in list(self.unreferencedNotes.items()):
                    if info['def']:
                        atts = info['def']['atts']
                        content = info['def']['content']
